                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <!-- SUCCESS -->
                        @if (session()->has('success'))
                            <div class="absolute bg-green-400 flex justify-center w-full alerta transition duration-1000 ease-in-out">
                                <h3 class="text-gray-600 text-sm"> {{ session()->get('success') }}</h3>
                            </div>
                        @endif
                        @if (session()->has('borrado'))
                            <div class="absolute bg-red-400 flex justify-center w-full alerta transition duration-1000 ease-in-out">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                @forelse ($posts as $post)
                                    <?php $i++; ?>
                                    <tr class="{{ $i % 2 == 0 ? 'bg-slate-200' : 'bg-slate-300' }} hover:bg-gray-300 dark:hover:bg-gray-600 ">
                                        <td class="px-6 py-4">
                                            @if ($post->image_url)
                                                <img class="max-w-xs h-auto"
                                                    src="/storage/images/{{ $post->image_url }}" alt="" />
                                            @endif
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <div class="border flex gap-2 size-full" >
                                            ...
                                            <form class="border" method="POST" action="{{ route('post.delete', $post->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <input type="submit"
